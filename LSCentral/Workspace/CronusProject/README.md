...
The project can be built using a [build script](.\Build\Build.ps1), to produce both the app files and the packages and without involving a service tier in the process. It does so by downloading dependency apps from the Update Service server and uses them as symbol files.
...
When the build script has executed successfully, you can run a deployment script to import the packages into your Update Service server: 
```powershell
PS> .\Build\Depoy.ps1 -Target 'Dev' [-Server 'my-server.mydomain.local']
```
...
* Update Service client, configured to talk to your server.
* Update Service server cmdlets (the server does not need to be operational, nor have a database).
    * Package ID: *go-current-client*
* LS Package Tools
    * Package ID: *ls-package-tools*
* Your Update Service server must have the *bc-al-compiler* package.
    ```powershell
    Import-Module UpdateServiceServer
    Copy-UssPackageFromServer -Id 'bc-al-compiler' -SourceServer 'gocurrent.lsretail.com' -SourceUseSsl -SourceIdentity 'lsretail.com'
    ```
...
```
Where *-Server* points to your Update Service server.

## Test Environment

### Install Branch

If you have set up the build script to run for every push to a Git branch on a remote repository and continuously deploy the packages to your Update Service server, then you can install a snapshot of each branch:

```PowerShell
PS> .\Scripts\InstallDevBranch.ps1 -BranchName 'my-branch'
```

Note: You have to have the Update Service client installed on the machine and configured to talk to your server.
